...
use crate::codegen::LuaDocDeclarations;
...
impl<'a> Generator<'a> {
    ...
    fn type_reference(&self) -> Document<'a> {
        if self.luadoc == LuaDocDeclarations::None {
            return Document::Str("");
        }
        ...
        let module = self
            ...
            .split('/')
            .last()
            .expect("Lua generator could not identify imported module name.");
    ...
    pub fn compile(&mut self) -> Output<'a> {
        ...
        // Determine what Lua imports we need to generate
        ...
        // Generate Lua code for each statement
        ...
        if imports.is_empty() && statements.is_empty() {
            ...
            Ok(docvec![
                type_reference,
                imports.into_doc(LuaCodegenTarget::Lua)
            ])
        } else {
            Ok(docvec![
                type_reference,
    ...
    pub fn statement(&mut self, statement: &'a TypedDefinition) -> Option<Output<'a>> {
        match statement {
            ...
            Definition::Function(function) => {
                // If there's an external Lua implementation then it will be imported,
                // so we don't need to generate a function definition.
                if function.external_javascript.is_some() {
                    return None;
                }

                // If the function does not support Lua then we don't need to generate
                // a function definition.
                if !function.implementations.supports(Target::Lua) {
                    return None;
                }
    ...
    fn module_function(&mut self, function: &'a TypedFunction) -> Option<Output<'a>> {
        ...
        let body = match generator.function_body(&function.body, function.arguments.as_slice()) {
            ...
            // There is an error coming from some expression that is not supported on Lua
...
pub fn module(
    ...
    let document = Generator::new(line_numbers, module, target_support, luadoc)
        .compile()
        .map_err(|error| crate::Error::Lua {
...
pub fn ts_declaration(
    ...
    let document = luadoc::LuaDocGenerator::new(module)
        .compile()
        .map_err(|error| crate::Error::Lua {
            path: path.to_path_buf(),
            src: src.clone(),
...
fn is_usable_js_identifier(word: &str) -> bool {
    !matches!(
        ...
        // https://developer.mozilla.org/en-US/docs/Web/Lua/Reference/Lexical_grammar
        "await"
            | "arguments"
            | "break"
            | "case"
            ...
            // used as a `thenable` in Lua when the module is imported
